...
void SplitOneFile(const char* fullpath, const char* outdir,
    ...
    for (int i = 0; i < N; ++i) {
        ...
        TTree *newtree = tree->CopyTree("", "", nToCopy, first);
        newtree->Write();
        fout.Close();
...
void splitRoot(const char* folder, const char* outdir, const char* treename = "tree") {
    ...
    if (gSystem->AccessPathName(outdir)) {   // returns true if it does NOT exist
        gSystem->mkdir(outdir, kTRUE);       // kTRUE = create parents as needed
